...
**Raspberry Pi 5 + voice-bridge + OpenClaw + VOICEVOX + Ollama**
...
1. [システム概要](#1-システム概要)
2. [前提条件](#2-前提条件)
3. [セットアップ手順 (9ステップ)](#3-セットアップ手順)
4. [LLM プロバイダ設定](#4-llm-プロバイダ設定)
5. [設定ファイルリファレンス](#5-設定ファイルリファレンス)
6. [サービス起動と動作確認](#6-サービス起動と動作確認)
7. [セキュリティガイドライン](#7-セキュリティガイドライン)
8. [Cloudflare Tunnel による外部アクセス](#8-cloudflare-tunnel-による外部アクセス)
9. [トラブルシューティング](#9-トラブルシューティング)
10. [macOS テスト環境](#10-macos-テスト環境)
...
| Ollama | ローカル LLM ランタイム。オフライン・無料で推論可能 | 11434 |
...
| Ollama | 公式スクリプト | `curl -fsSL https://ollama.ai/install.sh \| bash` |
...
```bash
# setup.sh で自動インストール済み
# モデル一覧の確認
ollama list

# 推奨モデル (Pi 5 8GB)
ollama pull qwen2.5:7b          # 汎用 (ツール呼び出し対応)
ollama pull deepseek-r1:7b      # 推論特化
ollama pull gemma2:2b            # 超軽量 (メモリ不足時)
...
```bash
# API キー取得: https://openrouter.ai/keys
...
モデル参照形式は `openrouter/<provider>/<model>` です。主なモデル:

| モデル | 参照名 | 用途 |
|---|---|---|
| Claude Sonnet 4.5 | `openrouter/anthropic/claude-sonnet-4-5` | 高品質汎用 |
...
curl -s http://127.0.0.1:18789/v1/chat/completions \
  ...
  -d '{"model":"openclaw","messages":[{"role":"user","content":"テストなのだ"}]}'
```
...
### 5.5 Docker sandbox (docker-compose.yml)
...
2. LAN に公開しない
3. 外部アクセスは SSH トンネル、Tailscale、または Cloudflare Tunnel 経由のみ (→ [第8章](#8-cloudflare-tunnel-による外部アクセス))
4. OpenClaw は専用 OS ユーザー (aiagent) で実行
5. 危険処理は必ず Docker sandbox 経由
6. 定期的にアップデートを実施

### 7.2 systemd セキュリティ設定
...
### 8.9 Cloudflare Zero Trust によるアクセス制御 (必須)
